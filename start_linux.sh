...
taskset -c 0-47,96-143 /usr/local/qemu/bin/qemu-system-x86_64 \
    ...
    -append 'console=ttyS0 rdinit=/usr/bin/busybox mitigations=off hugepages=0 transparent_hugepage=never SHELL=/bin/sh LOGNAME=root HOME=/ USER=root PATH=/bin:/benchmark -- sh -l' \
    -nographic
