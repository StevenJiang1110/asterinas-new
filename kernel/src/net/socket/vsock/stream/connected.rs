...
use crate::{
    ...
    util::{ring_buffer::RingBuffer, IoVecRead, IoVecWrite},
};
...
impl Connected {
    ...
    pub fn try_recv(&self, writer: &mut dyn IoVecWrite) -> Result<usize> {
    ...
    pub fn send(&self, reader: &mut dyn IoVecRead, flags: SendRecvFlags) -> Result<usize> {
        let mut connection = self.connection.disable_irq().lock();
        debug_assert!(flags.is_all_supported());
        let buf_len = reader.len();
        VSOCK_GLOBAL
            .get()
            .unwrap()
