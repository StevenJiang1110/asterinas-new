...
use crate::{events::IoEvents, prelude::*, return_errno_with_message, util::IoVecRead};
...
impl VsockSpace {
    ...
    /// Send a data packet
    pub fn send(&self, reader: &mut dyn IoVecRead, info: &mut ConnectionInfo) -> Result<()> {
        // FIXME: Creating this buffer should be avoided
        // if the underlying driver can accept reader.
        let mut buffer = vec![0u8; reader.len()];
        reader.read_exact(&mut VmWriter::from(buffer.as_mut_slice()))?;
