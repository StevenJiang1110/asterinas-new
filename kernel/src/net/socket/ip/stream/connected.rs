...
impl ConnectedStream {
    ...
    pub fn try_recv(&self, writer: &mut dyn IoVecWrite, _flags: SendRecvFlags) -> Result<usize> {
        let result = self.bound_socket.raw_with(|socket: &mut RawTcpSocket| {
            socket.recv(|socket_buffer| {
                match writer.write_all(&mut VmReader::from(&*socket_buffer)) {
                    Ok(len) => (len, Ok(len)),
                    Err(e) => (0, Err(e)),
                }
            })
    ...
    pub fn try_send(&self, reader: &mut dyn IoVecRead, _flags: SendRecvFlags) -> Result<usize> {
        let result = self.bound_socket.raw_with(|socket: &mut RawTcpSocket| {
            socket.send(|socket_buffer| {
                match reader.read_exact(&mut VmWriter::from(socket_buffer)) {
                    Ok(len) => (len, Ok(len)),
                    Err(e) => (0, Err(e)),
                }
            })
