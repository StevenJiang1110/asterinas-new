...
impl RingBuffer<u8> {
    ...
    pub fn write_fallible(&mut self, reader: &mut dyn IoVecRead) -> Result<usize> {
        let mut producer = Producer {
            rb: self,
            phantom: PhantomData,
        };
    ...
    pub fn read_fallible(&mut self, writer: &mut dyn IoVecWrite) -> Result<usize> {
        let mut consumer = Consumer {
            rb: self,
            phantom: PhantomData,
        };
