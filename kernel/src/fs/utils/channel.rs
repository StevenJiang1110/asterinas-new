...
impl Producer<u8> {
    ...
    pub fn try_write(&self, reader: &mut dyn IoVecRead) -> Result<usize> {
        if reader.is_empty() {
...
impl Consumer<u8> {
    ...
    pub fn try_read(&self, writer: &mut dyn IoVecWrite) -> Result<usize> {
        if writer.is_empty() {
            return Ok(0);
        }
...
impl<R: TRights> Fifo<u8, R> {
    #[require(R > Read)]
    pub fn read(&self, writer: &mut dyn IoVecWrite) -> Result<usize> {
        let mut rb = self.common.consumer.rb();
        rb.read_fallible(writer)
    }

    #[require(R > Write)]
    pub fn write(&self, reader: &mut dyn IoVecRead) -> Result<usize> {
